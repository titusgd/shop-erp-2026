...
function initCustomersPage(root) {
    ...
    const renderRows = (customers) => {
        if (!customers.length) {
            tableBody.innerHTML =
                '<tr><td colspan="7" class="px-4 py-8 text-center text-slate-500 sm:px-5">目前沒有客戶資料</td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = customers
            .map(
                (customer) => `
                    <tr>
                        <td class="px-4 py-3 font-medium text-slate-900 sm:px-5">${escapeHtml(customer.code || '—')}</td>
                        <td class="px-4 py-3 text-slate-900 sm:px-5">${escapeHtml(customer.name)}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(customer.tax_id || '—')}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(customer.contact_name || '—')}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(customer.phone || '—')}</td>
                        <td class="px-4 py-3 sm:px-5">${statusBadge(customer.is_active)}</td>
                        <td class="px-4 py-3 text-right sm:px-5">
                            <div class="inline-flex items-center gap-1">
                                <a href="/customers/${customer.id}" class="rounded-lg px-2.5 py-1.5 text-sm font-medium text-slate-700 transition hover:bg-slate-50">檢視</a>
                                <a href="/customers/${customer.id}/edit" class="rounded-lg px-2.5 py-1.5 text-sm font-medium text-teal-700 transition hover:bg-teal-50">編輯</a>
                                <button
                                    type="button"
                                    data-action="delete"
                                    data-id="${customer.id}"
    ...
    const loadCustomers = async () => {
        ...
        try {
            ...
            from = payload.meta?.from ?? (customers.length ? 1 : 0);
            to = payload.meta?.to ?? customers.length;
            ...
            pagination.hidden = true;
            showAlert(error.message || '載入客戶列表失敗', 'error');
        }
    };
    ...
    const openDeleteDialog = (id, name, code) => {
        ...
        deleteDialogMessage.textContent = `確定要刪除客戶「${label}」嗎？此操作無法復原。`;
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api(`/api/customers/${pendingDeleteId}`, { method: 'DELETE' });
            pendingDeleteId = null;
            isDeleting = false;
            closeDeleteDialog();
            showAlert('客戶已刪除');
