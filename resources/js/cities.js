...
function initCitiesPage(root) {
    ...
    const renderRows = (cities) => {
        if (!cities.length) {
            tableBody.innerHTML =
                '<tr><td colspan="4" class="px-4 py-8 text-center text-slate-500 sm:px-5">目前沒有縣市資料</td></tr>';
            return;
        }

        tableBody.innerHTML = cities
            .map(
                (city) => `
                    <tr>
                        <td class="px-4 py-3 font-medium text-slate-900 sm:px-5">${escapeHtml(city.code || '—')}</td>
                        <td class="px-4 py-3 text-slate-900 sm:px-5">${escapeHtml(city.name)}</td>
                        <td class="px-4 py-3 sm:px-5">${statusBadge(city.is_active)}</td>
                        <td class="px-4 py-3 text-right sm:px-5">
                            <div class="inline-flex items-center gap-1">
                                <a href="/cities/${city.id}/edit" class="rounded-lg px-2.5 py-1.5 text-sm font-medium text-teal-700 transition hover:bg-teal-50">編輯</a>
                                <button
                                    type="button"
                                    data-action="delete"
                                    data-id="${city.id}"
                                    data-name="${escapeHtml(city.name)}"
                                    data-code="${escapeHtml(city.code || '')}"
    ...
    const loadCities = async () => {
        ...
        try {
            ...
            from = payload.meta?.from ?? (cities.length ? 1 : 0);
            to = payload.meta?.to ?? cities.length;
            total = payload.meta?.total ?? cities.length;
            ...
            pagination.hidden = true;
            showAlert(error.message || '載入縣市列表失敗', 'error');
        }
    };
    ...
    const openDeleteDialog = (id, name, code) => {
        ...
        deleteDialogMessage.textContent = `確定要刪除縣市「${label}」嗎？此操作無法復原。`;
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api(`/api/cities/${pendingDeleteId}`, { method: 'DELETE' });
            pendingDeleteId = null;
            isDeleting = false;
            closeDeleteDialog();
            showAlert('縣市已刪除');
...
const page = document.querySelector('[data-cities-page]');
if (page) {
    initCitiesPage(page);
}
