...
function collectProductCategoryPayload(form) {
    return {
...
function initProductCategoryEditPage(root) {
    const productCategoryId = root.dataset.productCategoryId;
    const indexUrl = root.dataset.indexUrl;
    const form = root.querySelector('[data-product-category-form]');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const payload = collectProductCategoryPayload(form);
        submitButton.disabled = true;

        try {
            await api(`/api/product-categories/${productCategoryId}`, {
                method: 'PUT',
...
const page = document.querySelector('[data-product-category-edit-page]');
if (page) {
    initProductCategoryEditPage(page);
}
