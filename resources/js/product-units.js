...
function initProductUnitsPage(root) {
    ...
    const renderRows = (units) => {
        if (!units.length) {
            tableBody.innerHTML =
                '<tr><td colspan="5" class="px-4 py-8 text-center text-slate-500 sm:px-5">目前沒有商品單位資料</td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = units
            .map(
                (unit) => `
                    <tr>
                        <td class="px-4 py-3 font-medium text-slate-900 sm:px-5">${escapeHtml(unit.code || '—')}</td>
                        <td class="px-4 py-3 text-slate-900 sm:px-5">${escapeHtml(unit.name)}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(unit.symbol || '—')}</td>
                        <td class="px-4 py-3 sm:px-5">${statusBadge(unit.is_active)}</td>
                        <td class="px-4 py-3 text-right sm:px-5">
                            <div class="inline-flex items-center gap-1">
                                <a href="/product-units/${unit.id}/edit" class="rounded-lg px-2.5 py-1.5 text-sm font-medium text-teal-700 transition hover:bg-teal-50">編輯</a>
                                <button
                                    type="button"
                                    data-action="delete"
                                    data-id="${unit.id}"
                                    data-name="${escapeHtml(unit.name)}"
                                    data-code="${escapeHtml(unit.code || '')}"
    ...
    const loadProductUnits = async () => {
        ...
        try {
            const payload = await api(`/api/product-units?${params.toString()}`);
            const units = payload.data ?? [];

            page = payload.meta?.current_page ?? 1;
            lastPage = payload.meta?.last_page ?? 1;
            from = payload.meta?.from ?? (units.length ? 1 : 0);
            to = payload.meta?.to ?? units.length;
            total = payload.meta?.total ?? units.length;
            ...
            pagination.hidden = true;
            showAlert(error.message || '載入商品單位列表失敗', 'error');
        }
    };
    ...
    const openDeleteDialog = (id, name, code) => {
        ...
        deleteDialogMessage.textContent = `確定要刪除商品單位「${label}」嗎？此操作無法復原。`;
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api(`/api/product-units/${pendingDeleteId}`, { method: 'DELETE' });
            pendingDeleteId = null;
            isDeleting = false;
            closeDeleteDialog();
            showAlert('商品單位已刪除');
...
const page = document.querySelector('[data-product-units-page]');
if (page) {
    initProductUnitsPage(page);
}
