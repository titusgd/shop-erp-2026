...
function collectProductUnitPayload(form) {
    return {
        name: form.querySelector('[data-field="name"]').value.trim(),
        symbol: form.querySelector('[data-field="symbol"]').value.trim(),
        notes: form.querySelector('[data-field="notes"]').value.trim(),
        is_active: form.querySelector('[data-field="is_active"]').checked,
    };
}

function initProductUnitEditPage(root) {
    const productUnitId = root.dataset.productUnitId;
    const indexUrl = root.dataset.indexUrl;
    const form = root.querySelector('[data-product-unit-form]');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const payload = collectProductUnitPayload(form);
        submitButton.disabled = true;

        try {
            await api(`/api/product-units/${productUnitId}`, {
                method: 'PUT',
...
const page = document.querySelector('[data-product-unit-edit-page]');
if (page) {
    initProductUnitEditPage(page);
}
