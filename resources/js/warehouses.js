...
function initWarehousesPage(root) {
    ...
    const renderRows = (warehouses) => {
        if (!warehouses.length) {
            tableBody.innerHTML =
                '<tr><td colspan="7" class="px-4 py-8 text-center text-slate-500 sm:px-5">目前沒有倉庫資料</td></tr>';
            return;
        }
        ...
        tableBody.innerHTML = warehouses
            .map(
                (warehouse) => `
                    <tr>
                        <td class="px-4 py-3 font-medium text-slate-900 sm:px-5">${escapeHtml(warehouse.code || '—')}</td>
                        <td class="px-4 py-3 text-slate-900 sm:px-5">${escapeHtml(warehouse.name)}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(warehouse.contact_name || '—')}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(warehouse.phone || '—')}</td>
                        <td class="px-4 py-3 text-slate-700 sm:px-5">${escapeHtml(warehouse.address || '—')}</td>
                        <td class="px-4 py-3 sm:px-5">${statusBadge(warehouse.is_active)}</td>
                        <td class="px-4 py-3 text-right sm:px-5">
                            <div class="inline-flex items-center gap-1">
                                <a href="/warehouses/${warehouse.id}/edit" class="rounded-lg px-2.5 py-1.5 text-sm font-medium text-teal-700 transition hover:bg-teal-50">編輯</a>
                                <button
                                    type="button"
                                    data-action="delete"
                                    data-id="${warehouse.id}"
                                    data-name="${escapeHtml(warehouse.name)}"
                                    data-code="${escapeHtml(warehouse.code || '')}"
    ...
    const loadWarehouses = async () => {
        ...
        try {
            const payload = await api(`/api/warehouses?${params.toString()}`);
            const warehouses = payload.data ?? [];

            page = payload.meta?.current_page ?? 1;
            lastPage = payload.meta?.last_page ?? 1;
            from = payload.meta?.from ?? (warehouses.length ? 1 : 0);
            to = payload.meta?.to ?? warehouses.length;
            total = payload.meta?.total ?? warehouses.length;
            ...
            pagination.hidden = true;
            showAlert(error.message || '載入倉庫列表失敗', 'error');
        }
    };
    ...
    const openDeleteDialog = (id, name, code) => {
        ...
        deleteDialogMessage.textContent = `確定要刪除倉庫「${label}」嗎？此操作無法復原。`;
    ...
    const confirmDelete = async () => {
        ...
        try {
            await api(`/api/warehouses/${pendingDeleteId}`, { method: 'DELETE' });
            pendingDeleteId = null;
            isDeleting = false;
            closeDeleteDialog();
            showAlert('倉庫已刪除');
...
const page = document.querySelector('[data-warehouses-page]');
if (page) {
    initWarehousesPage(page);
}
