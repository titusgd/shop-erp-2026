...
async function loadCityOptions(select, selectedId = null) {
    ...
    cities.forEach((city) => {
        const selected = selectedId !== null && Number(selectedId) === Number(city.id);
        if (selected) {
            hasSelected = true;
        }
        options.push(
            `<option value="${city.id}"${selected ? ' selected' : ''}>${escapeHtml(city.name)}</option>`,
        );
    });

    // Keep current city available even if it is inactive.
    if (selectedId && !hasSelected) {
        try {
            const cityPayload = await api(`/api/cities/${selectedId}`);
            ...
            // Ignore load failure; validation will catch missing city.
        }
    }
...
function initDistrictEditPage(root) {
    ...
    loadCityOptions(citySelect, cityId).catch(() => {
    ...
    form.addEventListener('submit', async (event) => {
        ...
        try {
            await api(`/api/districts/${districtId}`, {
                method: 'PUT',
...
const page = document.querySelector('[data-district-edit-page]');
if (page) {
    initDistrictEditPage(page);
}
