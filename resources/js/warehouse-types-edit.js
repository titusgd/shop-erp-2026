...
function collectWarehouseTypePayload(form) {
    return {
...
function initWarehouseTypeEditPage(root) {
    const warehouseTypeId = root.dataset.warehouseTypeId;
    const indexUrl = root.dataset.indexUrl;
    const form = root.querySelector('[data-warehouse-type-form]');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const payload = collectWarehouseTypePayload(form);
        submitButton.disabled = true;

        try {
            await api(`/api/warehouse-types/${warehouseTypeId}`, {
                method: 'PUT',
...
const page = document.querySelector('[data-warehouse-type-edit-page]');
if (page) {
    initWarehouseTypeEditPage(page);
}
