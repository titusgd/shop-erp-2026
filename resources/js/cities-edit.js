...
function collectCityPayload(form) {
    return {
...
function initCityEditPage(root) {
    const cityId = root.dataset.cityId;
    const indexUrl = root.dataset.indexUrl;
    const form = root.querySelector('[data-city-form]');
    ...
    form.addEventListener('submit', async (event) => {
        ...
        const payload = collectCityPayload(form);
        submitButton.disabled = true;

        try {
            await api(`/api/cities/${cityId}`, {
                method: 'PUT',
...
const page = document.querySelector('[data-city-edit-page]');
if (page) {
    initCityEditPage(page);
}
