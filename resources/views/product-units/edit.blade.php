<x-layouts.app active="product-units">
    <x-slot:title>編輯商品單位</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-product-unit-edit-page
        data-product-unit-id="{{ $productUnit->id }}"
        data-index-url="{{ route('product-units.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">編輯商品單位</h2>
                <p class="mt-1 text-sm text-slate-500">更新單位名稱、簡稱與啟用狀態。</p>
            </div>
            <a
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            <form data-product-unit-form class="space-y-4">
                <div>
                    <label for="product-unit-code" class="mb-1.5 block text-sm font-medium text-slate-700">系統編號</label>
                    <input
                        id="product-unit-code"
                        type="text"
                        value="{{ $productUnit->code }}"
                        readonly
                ...
                <div>
                    <label for="product-unit-name" class="mb-1.5 block text-sm font-medium text-slate-700">單位名稱</label>
                    <input
                ...
                <div>
                    <label for="product-unit-symbol" class="mb-1.5 block text-sm font-medium text-slate-700">
                        簡稱
                        <span class="font-normal text-slate-400">（選填，不可重複）</span>
                    </label>
                    <input
                        id="product-unit-symbol"
                        name="symbol"
                        type="text"
                        maxlength="50"
                        data-field="symbol"
                        value="{{ old('symbol', $productUnit->symbol) }}"
                        placeholder="例如：pcs、ctn、kg"
                        class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
                    >
                    <p class="mt-1 hidden text-sm text-red-600" data-error="symbol"></p>
                </div>

                <div>
                    <label for="product-unit-notes" class="mb-1.5 block text-sm font-medium text-slate-700">
                        備註
                        <span class="font-normal text-slate-400">（選填）</span>
                    </label>
                    <textarea
                        id="product-unit-notes"
                        name="notes"
                        rows="3"
                        data-field="notes"
                        class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
                    >{{ old('notes', $productUnit->notes) }}</textarea>
                ...
                <div>
                    <label class="inline-flex items-center gap-2 text-sm font-medium text-slate-700">
                        <input
                            ...
                            value="1"
                            @checked(old('is_active', $productUnit->is_active))
    ...
    <x-slot:scripts>
        @vite('resources/js/product-units-edit.js')
    </x-slot:scripts>
</x-layouts.app>
