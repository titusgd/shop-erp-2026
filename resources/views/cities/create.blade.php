<x-layouts.app active="cities">
    <x-slot:title>新增縣市</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-city-create-page
        data-index-url="{{ route('cities.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">新增縣市</h2>
                <p class="mt-1 text-sm text-slate-500">填寫縣市名稱；儲存後將自動產生系統編號。</p>
            </div>
            <a
    ...
    <x-slot:scripts>
        @vite('resources/js/cities-create.js')
    </x-slot:scripts>
</x-layouts.app>
