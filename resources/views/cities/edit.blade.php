<x-layouts.app active="cities">
    <x-slot:title>編輯縣市</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-city-edit-page
        data-city-id="{{ $city->id }}"
        data-index-url="{{ route('cities.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">編輯縣市</h2>
                <p class="mt-1 text-sm text-slate-500">更新縣市名稱與啟用狀態。</p>
            </div>
            <a
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            <form data-city-form class="space-y-4">
                <div>
                    <label for="city-code" class="mb-1.5 block text-sm font-medium text-slate-700">系統編號</label>
                    <input
                        id="city-code"
                        type="text"
                        value="{{ $city->code }}"
                ...
                <div>
                    <label for="city-name" class="mb-1.5 block text-sm font-medium text-slate-700">縣市名稱</label>
                    <input
                        id="city-name"
                        name="name"
                        type="text"
                ...
                <div>
                    <label for="city-notes" class="mb-1.5 block text-sm font-medium text-slate-700">
                        備註
    ...
    <x-slot:scripts>
        @vite('resources/js/cities-edit.js')
    </x-slot:scripts>
</x-layouts.app>
