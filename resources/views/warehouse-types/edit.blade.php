<x-layouts.app active="warehouse-types">
    <x-slot:title>編輯倉庫類型</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-warehouse-type-edit-page
        data-warehouse-type-id="{{ $warehouseType->id }}"
        data-index-url="{{ route('warehouse-types.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">編輯倉庫類型</h2>
                <p class="mt-1 text-sm text-slate-500">更新類型名稱與啟用狀態。</p>
            </div>
            <a
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            <form data-warehouse-type-form class="space-y-4">
                <div>
                    <label for="warehouse-type-code" class="mb-1.5 block text-sm font-medium text-slate-700">系統編號</label>
                    <input
                ...
                <div>
                    ...
                    <input
                        ...
                        required
                        data-field="name"
                        value="{{ old('name', $warehouseType->name) }}"
                ...
                <div>
                    ...
                    <textarea
                        ...
                        data-field="notes"
                        class="block w-full rounded-lg border border-slate-300 bg-white px-3 py-2 text-sm text-slate-900 shadow-sm outline-none transition placeholder:text-slate-400 focus:border-teal-500 focus:ring-2 focus:ring-teal-500/20"
                    >{{ old('notes', $warehouseType->notes) }}</textarea>
                ...
                <div>
                    <label class="inline-flex items-center gap-2 text-sm font-medium text-slate-700">
                        <input
                            ...
                            value="1"
                            @checked(old('is_active', $warehouseType->is_active))
    ...
    <x-slot:scripts>
        @vite('resources/js/warehouse-types-edit.js')
    </x-slot:scripts>
</x-layouts.app>
