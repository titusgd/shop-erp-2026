<x-layouts.app active="purchase-orders">
    <x-slot:title>檢視採購單</x-slot:title>

    <div
        class="mx-auto max-w-5xl space-y-6"
        data-purchase-order-show-page
        data-purchase-order-id="{{ $purchaseOrder->id }}"
        data-index-url="{{ route('purchase-orders.index') }}"
        data-edit-url="{{ route('purchase-orders.edit', $purchaseOrder) }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">檢視採購單</h2>
                <p class="mt-1 text-sm text-slate-500">查看採購單主檔與明細。</p>
            </div>
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            <div data-purchase-order-detail class="space-y-5">
    ...
    <x-slot:scripts>
        @vite('resources/js/purchase-orders-show.js')
    </x-slot:scripts>
</x-layouts.app>
