<x-layouts.app active="product-categories">
    <x-slot:title>新增商品分類</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-product-category-create-page
        data-index-url="{{ route('product-categories.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">新增商品分類</h2>
                <p class="mt-1 text-sm text-slate-500">填寫分類名稱；儲存後將自動產生系統編號。</p>
            </div>
            <a
    ...
    <x-slot:scripts>
        @vite('resources/js/product-categories-create.js')
    </x-slot:scripts>
</x-layouts.app>
