<x-layouts.app active="customers">
    <x-slot:title>編輯客戶</x-slot:title>

    <div
        class="mx-auto max-w-2xl space-y-6"
        data-customer-edit-page
        data-customer-id="{{ $customer->id }}"
        data-index-url="{{ route('customers.index') }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">編輯客戶</h2>
                <p class="mt-1 text-sm text-slate-500">更新客戶資料。</p>
            </div>
            <a
        ...
        <section class="rounded-xl border border-slate-200 bg-white p-4 shadow-sm sm:p-8">
            <form data-customer-form class="space-y-4">
                <div>
                    <label for="customer-code" class="mb-1.5 block text-sm font-medium text-slate-700">系統編號</label>
                    <input
                        id="customer-code"
                        type="text"
                        value="{{ $customer->code }}"
                ...
                <div>
                    <label for="customer-name" class="mb-1.5 block text-sm font-medium text-slate-700">客戶名稱</label>
                    <input
                        id="customer-name"
                        name="name"
                        type="text"
                ...
                <div>
                    <label for="customer-tax-id" class="mb-1.5 block text-sm font-medium text-slate-700">
                        統一編號
                ...
                <div>
                    <label for="customer-contact-name" class="mb-1.5 block text-sm font-medium text-slate-700">
                        聯絡人
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    <div>
                        <label for="customer-phone" class="mb-1.5 block text-sm font-medium text-slate-700">
                            電話
                            <span class="font-normal text-slate-400">（選填）</span>
                        </label>
                        <input
                ...
                <x-address-location-fields
                    ...
                    :district-id="$customer->district_id"
                    :district-name="$customer->district?->name"
                />

                <div>
                    <label for="customer-address" class="mb-1.5 block text-sm font-medium text-slate-700">
                        地址
                        <span class="font-normal text-slate-400">（選填）</span>
                    </label>
                    <input
                ...
                <div>
                    <label for="customer-notes" class="mb-1.5 block text-sm font-medium text-slate-700">
                        備註
                        <span class="font-normal text-slate-400">（選填）</span>
                    </label>
                    <textarea
