<x-layouts.app active="purchase-requisitions">
    <x-slot:title>請購單</x-slot:title>

    <div class="mx-auto max-w-6xl space-y-6" data-purchase-requisitions-page>
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">請購單</h2>
                <p class="mt-1 text-sm text-slate-500">建立與管理請購單，指定請購人、進貨倉庫與請購明細。</p>
            </div>
            <a
            ...
            >
                ...
                <span class="whitespace-nowrap transition-all duration-200 sm:max-w-0 sm:overflow-hidden sm:opacity-0 sm:group-hover:max-w-[6rem] sm:group-hover:opacity-100 sm:group-focus-visible:max-w-[6rem] sm:group-focus-visible:opacity-100">
                    新增請購單
                </span>
            </a>
        </section>
            ...
            <div class="flex flex-col gap-3 border-b border-slate-200 p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5">
                <div class="flex w-full flex-col gap-3 sm:max-w-xl sm:flex-row">
                    <div class="relative w-full sm:flex-1">
                        <label for="purchase-requisition-search" class="sr-only">搜尋請購單</label>
                        <input
                            id="purchase-requisition-search"
                            type="search"
                            data-search
                            placeholder="搜尋單號、請購人或倉庫"
                    ...
                    <div class="w-full sm:w-40">
                        <label for="purchase-requisition-status-filter" class="sr-only">狀態篩選</label>
                        <select
                            id="purchase-requisition-status-filter"
                            data-status-filter
            ...
            <div class="overflow-x-auto">
                <table class="min-w-[48rem] w-full divide-y divide-slate-200 text-left text-sm">
                    <thead class="bg-slate-50 text-slate-500">
                        <tr>
                            <th class="whitespace-nowrap px-4 py-3 font-medium sm:px-5">請購單號</th>
                            <th class="whitespace-nowrap px-4 py-3 font-medium sm:px-5">請購日期</th>
                            <th class="whitespace-nowrap px-4 py-3 font-medium sm:px-5">請購人</th>
                            <th class="whitespace-nowrap px-4 py-3 font-medium sm:px-5">進貨倉庫</th>
                    ...
                    <tbody class="divide-y divide-slate-100" data-table-body>
                        <tr>
                            <td colspan="6" class="px-4 py-8 text-center text-slate-500 sm:px-5">載入中…</td>
                        </tr>
                    </tbody>
                </table>
            </div>
        ...
        <div
            ...
            role="dialog"
            aria-modal="true"
            aria-labelledby="purchase-requisition-delete-dialog-title"
            aria-hidden="true"
        >
            ...
            <div class="relative w-full max-w-md rounded-xl border border-slate-200 bg-white p-5 shadow-xl sm:p-6">
                <h3 id="purchase-requisition-delete-dialog-title" class="text-lg font-semibold text-slate-900">確認刪除</h3>
    ...
    <x-slot:scripts>
        @vite('resources/js/purchase-requisitions.js')
    </x-slot:scripts>
</x-layouts.app>
