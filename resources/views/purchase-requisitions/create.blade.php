<x-layouts.app active="purchase-requisitions">
    <x-slot:title>新增請購單</x-slot:title>

    <div
        class="mx-auto max-w-5xl space-y-6"
        data-purchase-requisition-create-page
        data-index-url="{{ route('purchase-requisitions.index') }}"
        data-current-user-id="{{ auth()->id() }}"
    >
        <section class="flex flex-col gap-4 sm:flex-row sm:items-end sm:justify-between">
            <div class="min-w-0">
                <h2 class="text-xl font-semibold tracking-tight text-slate-900 sm:text-2xl">新增請購單</h2>
                <p class="mt-1 text-sm text-slate-500">指定請購人與進貨倉庫，並填寫請購明細；儲存後將自動產生請購單號。</p>
            </div>
            <a
        ...
        @include('purchase-requisitions._form')
    </div>

    <x-slot:scripts>
        @vite('resources/js/purchase-requisitions-create.js')
    </x-slot:scripts>
</x-layouts.app>
